...
flowchart LR
    Vodic["🧍 Vodič"]:::actor
    ADAS["🧍 Palubný ADAS / OBD systém"]:::actor
    Tretia["🧍 Tretia strana (Polícia SR)"]:::actor

    subgraph SYS["Systém: SmartHUD"]
        UC1(["Zvoliť režim a HW výstup"]):::common
        UC2(["Zobraziť HUD odporúčanie úspornej jazdy (ECO)"]):::eco
        UC3(["Získať dopravné dáta"]):::eco
        UC4(["Aktivovať RaceHUD režim (trať)"]):::race
        UC5(["Synchronizovať s ACC"]):::common
        UC6(["Zaznamenať dlhodobé prekročenie limitu"]):::common
        UC7(["Sprístupniť záznam tretej strane"]):::common
    end
    ...
    classDef actor fill:#f7fafc,stroke:#4a5568,stroke-width:1px;
    classDef eco fill:#d4f7dc,stroke:#2f855a;
    classDef race fill:#fde2e1,stroke:#c53030;
    classDef common fill:#dbeafe,stroke:#2b6cb0;
```
...
```mermaid
sequenceDiagram
    actor Vodic as Vodič
    participant App as Aplikácia SmartHUD
    participant HUD as HUD Renderer
    participant GPS as GPS / mapové dáta
    participant Doprava as Dopravný server
    participant ADAS as ADAS / OBD-II
    participant Policia as Tretia strana (Polícia SR)

    Vodic->>App: 1. Spustiť aplikáciu, zvoliť režim a HW výstup
    App->>HUD: 2. Inicializovať HW výstup
    App->>GPS: 3. Požiadať polohu a trasu
    GPS-->>App: 4. poloha + trasa
    App->>Doprava: 5. Požiadať dopravné dáta
    Doprava-->>App: 6. kolóna/hustá premávka + meškanie
    App->>ADAS: 7. Požiadať rýchlosť, odstup, stav ACC
    ADAS-->>App: 8. rýchlosť + odstup + stav ACC

    loop počas jazdy
        App->>App: 9. Vypočítať odporúčanú rýchlosť a projekciu
        App->>HUD: 10. Vykresliť HUD projekciu
        HUD-->>App: 11. potvrdenie vykreslenia
        App-->>Vodic: 12. zobraziť HUD odporúčanie

        opt prekročenie je dlhodobé
            App->>App: 13. Označiť udalosť ako porušenie
            App-->>Vodic: 14. informovať: dlhodobé prekročenie limitu sa zaznamenáva
        end
    end

    opt podľa zmluvných podmienok a oprávnenia
        App->>Policia: 15. Odoslať šifrovaný záznam porušenia
        Policia-->>App: 16. potvrdenie záznamu
    end
```
...
classDiagram
    class Vodic {
        -id: String
        -meno: String
        -preferovanyRezim: RezimTyp
        +zvolRezim(rezim: RezimTyp) void
    }
    ...
    class Jazda {
        ...
        -casStart: DateTime
        -casKoniec: DateTime
        +vypocitajOdporucanie() OdporucaciaZona
        +zaznamenajPorusenie() ZaznamJazdy
    }
    ...
    class HUDRenderer {
        ...
        -geometria: String
        -idZariadenia: String
        +renderuj(zona: OdporucaciaZona) void
    }
    ...
    class ADASRozhranie {
        ...
        -stavACC: boolean
        -idVozidla: String
        +synchronizuj(jazda: Jazda) void
    }

    class UsekCesty {
        -id: String
        -rychlostnyLimit: double
        -suradnice: String
        +getLimit() double
    }
    ...
    class OdporucaciaZona {
        -id: String
        -typOdporucania: String
        -farbaGradientu: String
        +vypocitaj() void
    }

    class ZaznamJazdy {
        -id: String
        -trvaniePrekrocenia: int
        -nameranaRychlost: double
        -limit: double
        -stavOdoslania: String
        +sifruj() void
        +odosli(tretiaStrana: TretiaStrana) void
    }

    class TretiaStrana {
        -nazov: String
        -kontakt: String
        +prijmiZaznam(zaznam: ZaznamJazdy) void
    }

    Vodic "1" -- "0..*" Jazda : vykonal
    Jazda "1" -- "0..*" UsekCesty : prechádza
    Jazda "1" -- "1" ADASRozhranie : synchronizuje
    Jazda "1" -- "1" HUDRenderer : zobrazuje cez
    UsekCesty "1" -- "0..*" OdporucaciaZona : obsahuje
    UsekCesty "1" -- "0..*" DopravnaSituacia : má
    Jazda "1" -- "0..*" ZaznamJazdy : vytvára
    ZaznamJazdy "0..*" -- "1" TretiaStrana : sprístupní podľa zmluvných podmienok
```
...
```mermaid
flowchart TD
    Start([Štart]) --> A[Spustiť aplikáciu, zvoliť režim a HW]
    A --> B[Získať GPS polohu a rýchlosť]
    B --> C[Detekcia HW zobrazovača]
    C --> D[Kontrola zvoleného režimu]
    D --> E[Výpočet odporúčanej rýchlosti]
    E --> F[Vykreslenie projekcie]
    F --> G{Prekročenie limitu?}
    G -- Nie --> H[Pokračovať v jazde]
    G -- Áno --> I{Aktívny režim?}
    I -- ECO --> J{Prekročenie je dlhodobé?}
    J -- Áno --> K[Vytvoriť/aktualizovať záznam porušenia]
    K --> L[Zobraziť varovanie vodičovi: Spomaľte – prekračujete limit]
    L --> H
    J -- Nie --> H
    I -- PERFORMANCE alebo RACE --> M[Nezaznamenávať prekročenie]
    M --> H
    H --> N{Koniec jazdy?}
    N -- Nie --> B
    N -- Áno --> Koniec([Koniec])
...
```mermaid
flowchart TD
    subgraph Vodic["Dráha: Vodič"]
        V1([Štart: Nastúpiť a spustiť aplikáciu])
        V2[Zvoliť režim a HW výstup]
        V6([Koniec: Ukončiť jazdu])
    end

    subgraph App["Dráha: Aplikácia SmartHUD"]
        A1[Inicializovať HUD výstup]
        A2[Vypočítať odporúčanie]
        A3[Zobraziť HUD projekciu]
        A4{Prekročenie limitu?}
        A5{Aktívny režim = ECO?}
        A6[Zaznamenať prekročenie]
        A7[Ignorovať prekročenie Performance mód]
        A8[Pokračovať v monitorovaní]
        A9{Koniec jazdy?}
    end

    subgraph Vozidlo["Dráha: Vozidlo / ADAS"]
        AD1[Poskytnúť rýchlosť z CAN, odstup, stav ACC]
        AD2[Poskytnúť poveternostné podmienky a priľnavosť vozovky]
    end

    subgraph Externe["Dráha: Externé služby"]
        E1[Poskytnúť GPS polohu a trasu]
        E2[Poskytnúť dopravné dáta]
    end

    subgraph Policia["Dráha: Tretia strana - Polícia SR"]
        P1[Prijať šifrovaný záznam]
    end

    V1 --> V2 --> A1
    A1 -.->|message| E1 -.->|message| A2
    A1 -.->|message| E2 -.->|message| A2
    A1 -.->|message| AD1 -.->|message| A2
    A1 -.->|message| AD2 -.->|message| A2
    A2 --> A3 --> A4
    A4 -- Nie --> A8
    A4 -- Áno --> A5
    A5 -- Áno --> A6 --> A8
    A5 -- Nie --> A7 --> A8
    A6 -.->|message: podľa zmluvných podmienok a oprávnenia| P1
    A8 --> A9
    A9 -- Nie --> A2
    A9 -- Áno --> V6
```
